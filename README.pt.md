...
- `GET /api/account` (usuário atual), `PUT /api/account/password`, `DELETE /api/account` (trocar senha e deletar conta também exigem a senha atual)
...
- `GET /api/routes` lista toda rota `/api/*` registrada (método, URI, nome), conveniência de dev pra explorar o template, não é pra tráfego de produção
...
Docs OpenAPI gerados a partir de atributos `zircote/swagger-php` (`#[OA\...]`) nos controllers, via [L5-Swagger](https://github.com/DarkaOnLine/L5-Swagger). Com o app rodando, abre `/api/documentation` pro Swagger UI interativo: `http://localhost:8000/api/documentation` via `composer dev`, `http://localhost:8080/api/documentation` via Docker. Usa o botão "Authorize" com um token Sanctum de `/api/auth/login` pra testar as rotas protegidas (`notes`, `account`, `auth/logout`, `auth/email/resend`).
...
- **Requests sequenciais dentro de um teste Pest podem autenticar com um token já revogado**: o `AuthManager` do Laravel cacheia instâncias de guard resolvidas (e `RequestGuard` cacheia seu usuário resolvido nele mesmo) pela vida da app. Isso é ok em produção, onde toda request real inicializa uma app nova, mas dentro de um teste fazendo várias requests pela mesma app já inicializada, um token revogado pela request *N* ainda podia "autenticar" na request *N+1*. Não é bug de produção, mas real o suficiente pra quebrar 3 testes até `forgetAuthGuards()` (`tests/Pest.php`) ser chamado depois de toda request que muda estado de auth.
